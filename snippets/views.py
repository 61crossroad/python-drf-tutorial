...
# DefaultRouter alternates api_root
@api_view(['GET'])
...
# already mixed-in generic views
...
# similar for any model-backend API views, common behaviours in mixins
...
# basic class-based views similar to the functional views
class SnippetApiViewList(APIView):
...
class SnippetApiViewDetail(APIView):
...
# @api_view
@api_view(['GET', 'POST'])
def snippet_api_view_list(request, format=None):
...
def snippet_api_view_detail(request, pk, format=None):
    try:
